import cv2
import numpy as np
from preprocessing.image_validator import validate_blur
...
def test_detect_blur_sharp_image():
    ...
    is_blur, score = validate_blur(img, threshold=100.0)
    assert not is_blur
    assert score > 100.0
...
def test_detect_blur_blurred_image():
    ...
    is_blur, score = validate_blur(blurred, threshold=100.0)
    assert is_blur
    assert score < 100.0
